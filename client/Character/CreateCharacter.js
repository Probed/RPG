...
define([
	...
	RPG.CreateCharacter = new Class({
		...
		this.createDiv = new Element('div',{
			 ...
			 * Delegated Events :
			 */
			'click:relay(span.createCharacter)' : function(event) {
			    this.createCharacter();
			}.bind(this),
			'click:relay(span.ResetCharacter)' : function(event) {

			},
			'click:relay(span.PrevPortrait)' : function(event) {
			    this.options.portraitIndex-=1;
			    this.updatePortraits();
			}.bind(this),
			'click:relay(span.NextPortrait)' : function(event) {
			    this.options.portraitIndex+=1;
			    this.updatePortraits();
			}.bind(this),
			...
			'click:relay(span.RandomDiff)' : this.randFuncs[r++] = function(event) {
			    this.diffSelect.value = Object.getSRandom(RPG.Difficulty).key;
			    this.resetStats();
			}.bind(this),
			'click:relay(span.RandomName)' : this.randFuncs[r++] = function(event) {
			...
			'click:relay(span.RandomGender)' : this.randFuncs[r++] = function(event) {
			    this.genderSelect.value = Object.getSRandom(RPG.Gender).key;
			    this.updatePortraits();
			    this.resetStats();
			}.bind(this),
			'click:relay(span.RandomPortrait)' : this.randFuncs[r++] = function(event) {
			    this.options.portraitIndex = Math.floor(RPG.Random.random(0,Object.keys(RPG.Portraits[this.genderSelect.value]).length));
			    this.updatePortraits();
			}.bind(this),
			'click:relay(span.RandomRace)' : this.randFuncs[r++] = function(event) {
			    this.raceSelect.value = Object.getSRandom(RPG.Race).key;
			    this.resetStats();
			}.bind(this),
			'click:relay(span.RandomClass)' : this.randFuncs[r++] =function(event) {
			    this.classSelect.value = Object.getSRandom(RPG.Class).key;
			    this.resetStats();
			}.bind(this),
			'click:relay(span.RandomDist)' : this.randFuncs[r++] =function(event) {
			    this.resetStats();
			...
			'click:relay(span.RandomAll)' : function(event) {
			...
			[
			{
				...
				this.tblPortrait = new HtmlTable({
					...
					content : new Jx.Button({
					    label : '&lt;'
					}).toElement().addClass('PrevPortrait')
				    },
				    {
					properties : {
					...
					content : new Jx.Button({
					    label : 'Random'
					}).toElement().addClass('RandomPortrait')
				    },
				    {
					properties : {
					...
					content : new Jx.Button({
					    label : '&gt;'
					}).toElement().addClass('NextPortrait')
				    }
				    ]
			...
			},
			{
				...
				},
				{
				    ...
				    content : new Jx.Button({
					label : 'Random'
				    }).toElement().addClass('RandomGender')
				}
				],
				[
				{
				...
				},
				{
				    ...
				    content : new Jx.Button({
					label : 'Random'
				    }).toElement().addClass('RandomClass')
				}
				],
				[
				{
				...
				},
				{
					...
					Object.each(RPG.Stats,function(stats,name){
						...
						content : new Jx.Button({
						    label : '<span class="textLarge">-</span>',
						...
						content : new Jx.Button({
						    label : '<span class="textMedium">+</span>',
						    onClick : function(event) {
							var s = this.distributedStats + 1;
							var n = Number.from($(name).value);
							//determine the minimum number this Stat can have.
			...
			[
			{
			    ...
			    }).toElement().addClass('ResetCharacter')
			},
			{
			    properties : {
				'class' : 'textMedium textCenter',
			    ...
			    }).toElement().addClass('createCharacter')
			}
			]
			]
		    })
		...
		var character = {
		    Gender : gender,
		    portrait :RPG.Portraits[gender][Object.keys(RPG.Portraits[gender])[this.options.portraitIndex]] || RPG.Portraits[gender][0]
		};
		...
		if (!RPG.Portraits[gender][Object.keys(RPG.Portraits[gender])[this.options.portraitIndex]]) {
